...
τ = [-1.0, -3.2, -2.0, -1.5, -0.75, -0.5, -1.0, -2.0, -1.5, -1.0];
ψ = @formula ◊(sₜ -> sₜ > 0);
ψ(τ)
\end{juliaconsole}
\onslide<1-2>{\lineblackout{4}}
\onslide<1-3>{\lineblackout{3}}
\onslide<1-5>{\lineblackout{2}}
\onslide<1-7>{\lineblackout{1}\lineblackout{0}}
\onslide<8>{}
\end{footnotesize}

\begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south east, visible on=<4->] at ($(current page.south east)+(0.5cm,0)$) {
        \begin{jlcode}
            p = let
                times = collect(1:10)
                τ = [-1.0, -3.2, -2.0, -1.5, -0.75, -0.5, -1.0, -2.0, -1.5, -1.0];
...
% \begin{tikzpicture}[remember picture, overlay]
%     \node[visible on=<5->] at ($(current page.center)+(-3.0cm,-1.0cm)$) {\jlv{◊ # \lozenge<TAB>}};
% \end{tikzpicture}
